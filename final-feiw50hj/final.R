...
name <- "Kaifei Guo"
github.name <- "gkaifei"
email.address <- "guokaifei@berkeley.edu"
...
set.seed(42)
#z <- <your code here>
z <- sample(c(TRUE, FALSE), prob=c(0.7,0.3), size=5000, replace= TRUE)
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
#v <- <your code here>
v <- sample(1:6, prob=c(1/6,1/6,1/6,1/6,1/6,1/6), size=100, replace= TRUE)
...
# Diet 1 at time 0
#cw1 <- <your code here>
cw1 <- subset(ChickWeight, ChickWeight$Time==0 & ChickWeight$Diet==1)
  
# [1 pt]
# Create [cw2] a subset of ChickWeight with only Chicks that
# weighed more than 41 at Time 0
#cw2 <- <your code here>
cw2 <- subset(ChickWeight, ChickWeight$Time==0 & ChickWeight$weight>41)
...
sampleIndex <-as.numeric(sample(rownames(ChickWeight), size=200, replace=FALSE))
cw4 <- ChickWeight[sampleIndex, ]
...
# 
# t2 <- <your code here>
t2mid<-subset(infants, infants$ed=="College")
t2 <- table(t2mid$smoke, t2mid$number)
...
infan35older <-mean(infants$bwt[infants$age>=35], na.rm=TRUE)
infan35youner <- mean(infants$bwt[infants$age<35], na.rm=TRUE)
bdiff <-infan35older-infan35youner
...
amNew <- gsub("0", "2", mtcars$am)
mtcars <- cbind(mtcars, amNew)

plot(mtcars$disp~mtcars$wt, xlab="Weigth of the Car", ylab="Engine Displacement", col=mtcars$amNew)
...
plot(WorldBank$life.expectancy~WorldBank$fertility.rate, pch=".", col=as.numeric(WorldBank$region), 
...
subset1960 <- subset(WorldBank, WorldBank$year==1960)
subset2013 <- subset(WorldBank, WorldBank$year==2013)
plot(subset1960$life.expectancy~subset1960$fertility.rate, pch=".", col=as.numeric(subset1960$region), 
     xlab="fertility.rate", ylab="life.expectancy", main="1960")
plot(subset2013$life.expectancy~subset2013$fertility.rate, pch=".", col=as.numeric(subset2013$region), 
     xlab="fertility.rate", ylab="life.expectancy", main="2013")
...
subsetIDA <- subset(WorldBank, WorldBank$lending=="IDA")
hist(subsetIDA$GDP, breaks=7000/250, xlab="GDP", prob=T, main="IDA countries' GDP")
...
GenNorm <- function(mean, sd, n, plot.hist=TRUE){
  if(plot.hist==TRUE){
    random <- rnorm(n=n, mean=mean, sd=sd)
    hist(random,prob=T)
    return (random)} 
  else if (plot.hist==FALSE){
    random <- rnorm(n=n, mean=mean, sd=sd)
    return(random)}
}
